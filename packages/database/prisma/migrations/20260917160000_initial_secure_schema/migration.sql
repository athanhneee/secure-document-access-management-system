-- Initial Prisma migration derived from the canonical SQL and DBML.
-- PostgreSQL 18; PostgreSQL-native constraints remain authoritative.
-- Enhancements approved by ADR-006 are marked DATABASE-EVOLUTION.
-- PRISMA-UNREPRESENTABLE: CHECK constraints, partial indexes and triggers in this file
-- are intentional database guarantees and must not be removed by schema regeneration.
...
CREATE TABLE documents (
    ...
    created_at          TIMESTAMPTZ NOT NULL DEFAULT now(),
    updated_at          TIMESTAMPTZ NOT NULL DEFAULT now(),
    version             INTEGER NOT NULL DEFAULT 0 CHECK (version >= 0), -- DATABASE-EVOLUTION: optimistic concurrency
...
CREATE TABLE audit_logs (
    ...
    chain_partition     VARCHAR(80) NOT NULL DEFAULT 'GLOBAL', -- DATABASE-EVOLUTION: HMAC-chain sharding key
    chain_sequence      BIGINT NOT NULL, -- Assigned by trusted audit writer inside its transaction
    hmac_key_version    INTEGER NOT NULL DEFAULT 1, -- Key material remains outside the database
    chain_algorithm     VARCHAR(20) NOT NULL DEFAULT 'HMAC-SHA-256',
    CHECK (actor_user_id IS NOT NULL OR actor_username IS NOT NULL),
    CHECK (previous_hash IS NULL OR previous_hash ~ '^[0-9A-Fa-f]{64}$'),
    CHECK (entry_hash ~ '^[0-9A-Fa-f]{64}$'),
    CHECK (chain_sequence > 0),
    CHECK (hmac_key_version > 0),
    CHECK (chain_algorithm = 'HMAC-SHA-256'),
    CHECK ((chain_sequence = 1 AND previous_hash IS NULL) OR (chain_sequence > 1 AND previous_hash IS NOT NULL))
);
...
-- DATABASE-EVOLUTION: positions and HMAC values are unique within a chain partition.
CREATE UNIQUE INDEX uq_audit_chain_position ON audit_logs(chain_partition, chain_sequence);
CREATE UNIQUE INDEX uq_audit_chain_entry_hash ON audit_logs(chain_partition, entry_hash);
...
-- PRISMA-UNREPRESENTABLE: updated_at, cross-row ownership/clearance, audit continuity
-- and append-only enforcement require PostgreSQL trigger functions.
...
-- DATABASE-EVOLUTION: verify link continuity without storing or computing the HMAC key in PostgreSQL.
-- The trusted writer computes entry_hash = HMAC-SHA-256(canonical event + previous_hash) outside the database.
CREATE OR REPLACE FUNCTION validate_audit_chain_link() RETURNS trigger AS $$
DECLARE
...
BEGIN
  ...
  IF NEW.chain_sequence = 1 THEN
    IF EXISTS (SELECT 1 FROM audit_logs WHERE chain_partition = NEW.chain_partition) THEN
      RAISE EXCEPTION 'audit chain already initialized for partition';
    END IF;
    RETURN NEW;
  END IF;
  ...
  IF expected_previous_hash IS NULL OR NEW.previous_hash IS DISTINCT FROM expected_previous_hash THEN
    RAISE EXCEPTION 'audit chain previous_hash mismatch';
  END IF;
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;

CREATE TRIGGER trg_validate_audit_chain_link
BEFORE INSERT ON audit_logs
FOR EACH ROW EXECUTE FUNCTION validate_audit_chain_link();
